...
```{r q8, eval=FALSE, include=TRUE}
m$mapreduce(
  # map = "function(){emit(Math.floor(this.pop/100)*100, 1)}",
    map = "function(){emit({state: this.state, city: this.city}, this.pop)}",
  reduce = "function(key, pop){return Array.sum(pop)}"
)
```
